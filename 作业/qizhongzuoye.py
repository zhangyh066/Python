...
print("=" * 60)
# 把预测信号做成每月调仓的多空策略后，得到一组“看似优于买入持有”的统计结果，但拆解后可发现其经济来源有限：
# 1. 年化收益仅比基准高 1.3 个百分点，却承担几乎相同的波动（20.6% vs 21.3%），夏普提升主要靠“少亏一点”而非“多赚一段”。
# 2. 最大回撤与基准完全一致（−47%），说明策略在极端下跌月并未有效避险；收益优势来自日常小幅择时，而非尾部风险规避。
# 3. 命中率 53.7%、方向准确率 53.9%，均略高于 50%，但统计上仅对应每月 3.7% 的边际正确率，勉强优于抛硬币。
# 4. 累计收益领先全部发生在少数月份，且这些月份往往伴随高波动；策略实质是“在市场震荡期略微降低仓位”，从而少亏而非多赚。
# 综合来看，该 Timing 策略的“超额收益”更多是对波动节奏的随机匹配，而非对风险溢价的可靠预测；扣除交易成本、滑点与资金占用后，净收益优势将迅速消失，不具备可持续的资产配置价值。
...
print("=" * 100)
# 1. 没有任何模型在最大回撤上优于基准，全部落在 −47% 附近，说明所有信号对尾部风险均无效。
# 2. 年化收益差距主要源自“日常小幅择时”而非“避开大跌”：表现最好的 RV3+PD 组合仅把年化收益从 5.2% 提到 6.5%，提升 1.3 个百分点，但波动率同步压缩不到 1 个百分点，夏普提高至 0.31。
# 3. 单一变量策略全部跑输或勉强持平买入持有；加入 RV3 后多数组合收益下降，表明波动率信号在月频上主要提供噪声。
# 4. 胜率最高的 PD 单变量也仅 53.7%，RV3 单变量甚至低于 50%，边际正确率与抛硬币无异。
# 5. 多变量组合的收益优势集中在少数震荡月份，且这些月份对参数敏感；一旦训练窗口或信号阈值微调，超额收益迅速消散。
# 综合来看，统计差异主要来源于“随机少亏”而非“系统多赚”，在扣除交易滑点、资金占用与调仓成本后，净超额收益趋近于零，不具备可持续的战术配置价值。
...
# 季度维度上用滞后 RV3 预测市场收益，三项指标同步为负且幅度比月度更深：
# 1. R² −10.0%
# 预测误差方差比“历史均值”基准扩大一成，说明把上季波动率当作方向信号在季频上反而更糟。
# 2. ENC-NEW −1.01
# Clark-West 检验显著为负，直接拒绝“新增信息”假设；波动率不仅没带来方向性增益，还系统性地拟合了错误残差。
# 3. MSE-F −6.10
# Diebold-Mariano 损失差统计量远低于零，表明季均方预测误差显著大于基准，且负值绝对值大于月度结果，误差放大效应随持有期延长而加剧。
# 经济含义
# 季频下波动率与收益呈更微弱的反向关系，方向信号几乎消失；
# 三个月的聚合进一步放大了极端收益对平方损失的权重，导致预测精度急剧恶化；
# 投资者若按“上季波动高低”决定季度仓位，将系统性地承担更大预测误差，却无法获得对应的风险补偿，汇率、股息等慢变量亦无法挽救这一负向结果。